...
struct DistanceData {
		double			UTOF,	LTOF,	Track, Beta, Rig, Total;

		// Previously in the cout (probably added later):
		//rgdtTrue, betaTrue, etofTrue, etrdTrue, etrkTrue, rgdtDist, betaDist, etofDist, etrdDist, etrkDist, etofuDist, etofdDist, rgdtMeasured, betaMeasured, etofMeasured, etrdMeasured, etrkMeasured, sigma_rgdt->Eval(rgdtTrue), sigma_beta->Eval(betaTrue), sigma_etof->Eval(betaTrue), sigma_etrd->Eval(betaTrue), sigma_etrk->Eval(betaTrue), CurrentTOF, DRTof, CurrentTRD, <, DRTrd, CurrentTrack, DRTrk, etofuMeasured, etofDMeasured, etofHMeasured;
...
class DistanceMinimizer
{
		...
		double rgdtMeasured; 
		double betaMeasured; 
		double etofuMeasured;
		double etoflMeasured; 
		double etrkMeasured;
...
public:
													...
													etrkMeasured(0)
		{
				...
				sigma_etofl	= new TSpline3("Cubic Spline", Beta_cent,	sigmaEtofLinv,	 30);
		...
		void reset(Dst* dst){
				...
				// Ev initialisation, rigidity, beta
				AMSEventR* ev=dst -> ev;
				...
				// EdepTracker ; using A. Oliva's advice instead of EdepTrack
				std::vector<float> vEdepTrk(0);
				...
				// EdepTOF
				std::vector<float> vEdepTOF = EdepTOF(ev);
				etofuMeasured = ( vEdepTOF[0] + vEdepTOF[1] ) / 2;
				etoflMeasured = ( vEdepTOF[2] + vEdepTOF[3] ) / 2;
				//etofHMeasured = ( vEdepTOF[0] + vEdepTOF[1] + vEdepTOF[2]  + vEdepTOF[3] ) / 4;
		...
		DistanceData FindMinimum(TF1 * RvsB, bool bprot) 
		{
				...
				// Initialising 
				//distance.TRD	   = 1000000; 
				distance.UTOF	 = 1000000;
				distance.LTOF	 = 1000000; 
				...
				if (etofuMeasured <= 0) return distance;
				if (etoflMeasured <= 0) return distance;
				...
				for(double rgdtTrue = 0; rgdtTrue < step * 1E6; rgdtTrue += step)
				{
						...
						// Retrieving from the theoretical curves for this particle, the expected energies
						double etrkTrue  = fETrackBeta -> Eval(betaTrue);	
						double etofuTrue = EdepTOFUbeta -> Eval(betaTrue);
