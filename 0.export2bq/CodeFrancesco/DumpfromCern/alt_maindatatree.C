...
int main(int argc, char * argv[])
{
		...
		string istog= "/afs/cern.ch/work/f/fdimicco/private/Dimiccoli/Compiled/istogrammidati/istogramma"+ARGV+".root";
                string *file=&istog;	
                ...
                for(int i=(int)(2000)/1000*INDX;i<(int)(2000)/1000*(INDX+1);i++){	
            ...
            measure_stuff->Branch("Altitude",&Altitude);
		...
		if(ii%10000==0){
		    ...
		    if(contaeventi==0) {std::cout<<"RunType(>61442): "<<ev->fHeader.RunType<<" Zenith: "<<ev->fHeader.Zenith()<<std::endl;
                ...
                if(ev&&trig){
                    if(((trig->PhysBPatt&1)==1)&&!(((trig->PhysBPatt>>1)&1)==1)) Unbias=1;
                    else Unbias=0;
                }
                //selezione delle run
		    ...
		    Altitude=ev->fHeader.RadS;	
                ...
                if(minimumbiasTRACKER(ev,3)) minbiastrack=1;
		...
		Pres_Unbias=0;
		if(R_pre!=-1&&Beta_pre>0&&Qbest==1&&nPart==1&&Beta_pre<fb->Eval(R_pre)&&Beta_pre>fa->Eval(R_pre)){ 	
			Pres_Unbias=1;
			if(minimumbias(ev,3)&&golden(ev,severity,3)&&preselect(ev)) Preselected=1;
			else Preselected=0;
				}
	    /////////////////////////////////////////////////////////////////////////////////////
            measure_stuff->Fill();
