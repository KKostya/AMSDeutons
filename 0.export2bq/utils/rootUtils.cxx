...
namespace rootUtils{
  ...
  namespace root{
    ...
    TObject* getCanvasPrimitive( std::string fileName, std::string canvasName, std::string primitiveName ){
      TCanvas* canvas = (TCanvas*) rootUtils::root::get(fileName, canvasName);
      if( canvas == NULL ){
	...
	grNew -> SetPointEYlow(i, errYLow*scalingFactor);
	grNew -> SetPointEYhigh(i, errYHigh*scalingFactor);	
      }
  ...
  std::map< std::string, std::string > getEnergyBeamTestString(){
    ...
    std::map< std::string, float > energyBeamTest = rootUtils::getEnergyBeamTest();
  ...
  std::vector< std::string > splitFromEnd( std::string str, std::string delimiters ){
    std::vector< std::string > splittedString = rootUtils::split( str, delimiters );
